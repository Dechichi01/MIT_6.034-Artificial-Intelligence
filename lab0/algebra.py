...
class Sum(list, Expression):
    ...
    def add_if_possible(self):
        terms = []; result = 0
        for term in self:
            if isinstance(term, (int, float)):
                result+=term
            elif isinstance(term, Product) and len(term) is 1:
                result+=term[0]
            else:
                terms.append(term)

        if result is not 0: terms.append(result)
...
class Product(list, Expression):
    ...
    #Extra implementation
    def multiply_if_possible(self):
        terms = []; result = 1
        for term in self:
            if isinstance(term, (int, float)):
                result*=term
            elif isinstance(term, Sum) and len(term) is 1:
                result*=term[0]
            else:
                terms.append(term)

        if result is not 1: terms.append(result)
        return Product(terms)

#Extra implementation
def distributive(expr1, expr2):
    terms = []

    for term in expr1:
        terms.append( Product((term, expr2)).flatten().multiply_if_possible() )

    return Sum(terms).flatten().add_if_possible()
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):  
        expr1.flatten().add_if_possible()
        expr2.flatten().add_if_possible()      
        terms = []
        for term in expr1:            
            terms.append( distributive(expr2, Product([term])) )

        print( Sum(terms).flatten().add_if_possible() )
        
    if isinstance(expr1, Sum) and isinstance(expr2, Product): 
        return distributive(expr1, expr2)        

    if isinstance(expr1, Product) and isinstance(expr2, Sum): 
        return distributive(expr2, expr1)

    if isinstance(expr1, Product) and isinstance(expr2, Product): 
        return Product((expr1, expr2)).flatten().multiply_if_possible()

    #raise NotImplementedError


do_multiply( Sum(("x", 2)), Sum(("x",4)) )